...
from .nnunet_utils import sum_tensor, softmax_helper, sigmoid_helper


class RobustCrossEntropyLoss(nn.CrossEntropyLoss):
    """
    Just a compatibility layer since target tensor is float 
    and has an extra dimension.
    """
...
class RobustBinaryCrossEntropyLoss(nn.BCEWithLogitsLoss):
    """
    Just a compatibility layer since target tensor is float 
    and has an extra dimension.
    """
...
class DC_and_BCE_loss(nn.Module):
    def __init__(
        ...
        self.ce = RobustBinaryCrossEntropyLoss(**bce_kwargs)
        
        if not square_dice:
            self.dc = SoftDiceLoss(apply_nonlin=sigmoid_helper, 
                                   **soft_dice_kwargs)
        else:
            self.dc = SoftDiceLossSquared(apply_nonlin=sigmoid_helper, 
                                          **soft_dice_kwargs)
        ...
        name = type(self).__name__
        print(f'💠 {name} initiated with: \n'
              f'   dice_kws={soft_dice_kwargs}, \n'
              f'   square_dice={square_dice}, \n'
              f'   bce_kws={bce_kwargs}, \n'
              f'   dc_wt={weight_dice}, ce_wt={weight_ce}, '
              f'aggregate={aggregate}.')

    def forward(self, net_output, target):
        ## Multi-class BCE Support
        if net_output.ndim == target.ndim:
            if net_output.shape[1] != target.shape[1] and target.shape[1] == 1:
                # Change target to 1-hot
                input_target = target 
                target = torch.zeros_like(net_output).detach()
                target.scatter_(1, input_target.long(), 1)
        
        ## Compute Losses
        ce_loss = self.ce(net_output, target) if self.weight_ce != 0 else 0
        dc_loss = self.dc(net_output, target)['loss'] if self.weight_dice != 0 \
                  else 0
...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    if mask is not None:
        ...
        fn = torch.stack(tuple(x_i * mask[:, 0] for x_i in torch.unbind(fn, dim=1)), 
                         dim=1)
        tn = torch.stack(tuple(x_i * mask[:, 0] for x_i in torch.unbind(tn, dim=1)), 
                         dim=1)
