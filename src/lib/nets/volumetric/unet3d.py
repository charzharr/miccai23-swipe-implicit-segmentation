...
class UpTransition(nn.Module):
    def __init__(self, inChans, outChans, depth, act, bilinear=True):
        super(UpTransition, self).__init__()
        self.depth = depth
        
        if bilinear:
            assert inChans == outChans, f'Bilinear does not support this.'
            self.up_conv = nn.Upsample(scale_factor=2.0)
        else:
...
class UNet3d(BaseModel):
    ...
    def __init__(
        ...
        self.set_setting('base_channels', base_channels)
        self.set_setting('bilinear', bilinear)
        self.set_setting('act', act)

        self.down_tr64 = DownTransition(in_channels, 0, act)
        self.down_tr128 = DownTransition(base_channels, 1, act)
        self.down_tr256 = DownTransition(base_channels * 2, 2, act)
        self.down_tr512 = DownTransition(base_channels * 4, 3, act)

        self.up_tr256 = UpTransition(base_channels * 8, base_channels * 8, 
                                     2, act, bilinear=bilinear)
        self.up_tr128 = UpTransition(base_channels * 4, base_channels * 4, 
                                     1, act, bilinear=bilinear)
        self.up_tr64 = UpTransition(base_channels * 2, base_channels * 2, 
                                    0, act, bilinear=bilinear)
        self.out_tr = OutputTransition(base_channels, out_channels)
