...
class ResNetMed3D(BaseModel):
    ...
    def __init__(self, in_channels=3, classes=10,
        ...
        self.conv1 = nn.Sequential(
            *[nn.Conv3d(in_channels, self.in_planes, kernel_size=3, stride=2,
                       padding=1, bias=False),
             nn.BatchNorm3d(self.in_planes),
             nn.ReLU(inplace=True),
             nn.Conv3d(self.in_planes, self.in_planes, kernel_size=3, stride=1,
                       padding=1, bias=False)
            ]
        )
        ...
        self.segm = TranspConvNet(in_channels=512 * block.expansion, 
                                  classes=classes)
...
def generate_resnet3d(in_channels=1, classes=2, model_depth=18, **kwargs):
    ...
    res_net_dict = {10: [1, 1, 1, 1], 18: [2, 2, 2, 2], 34: [3, 4, 6, 3], 
