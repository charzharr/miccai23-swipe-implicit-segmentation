    ...
    def __init__(
        ...
        for i in range(num_upsamples):
            ...
            if self.use_deconv:
                ...
                if i == num_upsamples - 1 and reduce_last_dstride:
                    kernel, stride = (3, 4, 4), (1, 2, 2)
                modules_dict[f'scale{i+1}_up'] = nn.ConvTranspose3d(
                    in_channels, scale_out_dims, 
                ...
                if i == num_upsamples - 1 and reduce_last_dstride:
                    scale_factor = (1, 2, 2)
                modules_dict[f'scale{i+1}_up'] = nn.Upsample(
    ...
    def __init__(self, 
        ...
        if self.deep_sup:
            self.up2_deepsup = DeepSupBlock3d(1, 64, self.out_channels,
                self.norm, self.act,
                reduce_last_dstride=self.reduce_conv1_dstride
            )
