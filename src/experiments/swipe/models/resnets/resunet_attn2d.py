...
from ..unetr.unetr2d_pp import TransformerBlock
...
class ResUNetAttn2d(BaseModel):
    ...
    def __init__(self, 
        ...
        def attn_block(self, stage_idx):
            input_sizes = [96, 48, 24, 12]
            dims = self.stage_dims
            be = self.block_expansion
            
            proj_size = 32
            num_heads = 2
            print(f'Proj Size {proj_size}, {num_heads}')
            
            return TransformerBlock(
                input_size=input_sizes[stage_idx] ** 2,
                hidden_size=dims[stage_idx] * be,
                proj_size=proj_size,
                num_heads=num_heads,
                dropout_rate=0.1,
                pos_embed=True
