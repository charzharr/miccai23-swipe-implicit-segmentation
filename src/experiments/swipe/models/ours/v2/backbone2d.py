...
class ResNet2d(BaseModel):
    ...
    def __init__(self, 
        ...
        if shape_mapper == 'lite': 
            from .components2d import StageSeparatedShapeMapperLite2d
            self.shape_mapper = StageSeparatedShapeMapperLite2d(
                hidden_dim,
        ...
        # )
            ...
            from .components2d import ShapeMapperV22d
            self.shape_mapper = ShapeMapperV22d(
                hidden_dim,
                self.mconfig.embedding_dim,
                stage_dims=self.stage_dims, 
                block_expansion=self.block_expansion,
            ...
            from .components2d import ShapeMapperV32d
            self.shape_mapper = ShapeMapperV32d(
                hidden_dim,
                self.mconfig.embedding_dim,
                stage_dims=self.stage_dims, 
                block_expansion=self.block_expansion,
        ...
        if 'conv' in self.global_head:
            self.global_shape_mapper = nn.Sequential(
                ...
                nn.Conv2d(hidden_dim, self.global_embedding_dim, 1),
