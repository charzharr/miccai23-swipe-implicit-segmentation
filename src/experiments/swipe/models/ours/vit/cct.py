...
class ConvTokenizer(nn.Module):
    def __init__(self,
        ...
        from ...resnets import resnet2d
        resnet2d.BLOCK_EXPANSION = 2
        resnet2d.STAGE_DIMS = [64, 64, 64, 64]

        from lib.nets.component_factories import ActFactory, NormFactory2d
        from ...resnets.resnet2d import (
            ResNet2d, Bottleneck, Bottle2neck, BottleneckSE, Conv2d
        )

        self.conv_layers = ResNet2d(
            Bottle2neck, 
            [1, 1, 1, 1], 
            NormFactory2d('batchnorm'), 
            act_factory = ActFactory('relu'), 
            conv_layer=Conv2d,                  
            baseWidth=16,
    ...
    def forward(self, x):
        x = F.relu(self.conv_layers(x)['out'])
...
class GlobalDecoder(nn.Module):
    ...
    def __init__(self,
                 in_channels,
                 out_channels,
                 linear_out_length=32,
                 num_upsamples=2):
        super().__init__()
        
        self.fc = nn.Linear(in_channels, linear_out_length ** 2)
        
        decoder = [Rearrange('b hw -> b h w')]
        for i in range(num_upsamples):
            decoder.append(nn.ConvTranspose2d(64, 64, kernel_size=4, 
                                   stride=2, padding=1, bias=False))
            decoder.append(nn.BatchNorm2d(64))
            decoder.append(nn.ReLU(inplace=True))
        decoder.append(nn.Conv2d(64, out_channels, 1, 1, 0))
...
class TransformerClassifier(nn.Module):
    ...
    def __init__(self,
        ...
        if positional_embedding == 'none':
            ...
            self.positional_emb = nn.Parameter(sinusoidal_embedding(
                                                    sequence_length, 
                                                    embedding_dim),
                                                    requires_grad=False)
        ...
        # self.global_decoder = GlobalDecoder(embedding_dim, out_channels)
    ...
    def forward(self, x):
        ...
        # Forward Pass Transformer Layers
        mha_input = x[1:] if not self.seq_pool else x
        x = self.dropout(x)
        encoder_outs = []
        for blk in self.blocks:
            ...
            if self.seq_pool:
                ...
                encoder_outs.append(x[:, 1:])
        encoder_out = x

        # Output Processing: Sequential Pooling, etc
        x = self.norm(x)
        if self.seq_pool:
            attn_weights = rearrange(self.attention_pool(x), 'b n 1 -> b n') # Bx#crops
            global_out = einsum('b n, b n d -> b d', 
                                attn_weights.softmax(dim=1), x)  # BxEmbedDim
        else:
            global_out = x[:, 0]  # BxEmbedDim
            encoder_out = encoder_out[:, 1:]
        
        # x = self.fc(x)
        # glob_pred = self.global_decoder(x)
        
        # TODO: Global segmentation mask prediction
        
        return {
            'global_out': global_out, 
            'patch_out': encoder_out,
            'patch_outs': encoder_outs,
            'mha_in': mha_input
        }
...
class CCT(BaseModel):
    def __init__(
        ...
        *args, **kwargs
        ...
        self.set_setting('img_size', img_size)
        self.set_setting('embedding_dim', embedding_dim)
        self.set_setting('n_input_channels', n_input_channels)
        self.set_setting('n_output_channels', n_output_channels)
        self.set_setting('n_conv_layers', n_conv_layers)
        self.set_setting('conv_info', (kernel_size, stride, padding))
        self.set_setting('pool_info', (pooling_kernel_size, pooling_stride,
                                        pooling_padding))
        self.set_setting('seq_pool', seq_pool)
        self.set_setting('mlp_ratio', mlp_ratio)
        self.set_setting('num_heads', num_heads)
        self.set_setting('attention_dropout', attention_dropout)
        self.set_setting('stochastic_depth', stochastic_depth)
        ...
        self.classifier = TransformerClassifier(
            ...
            embedding_dim=embedding_dim,
            seq_pool=seq_pool,
            dropout_rate=0.0,                     # orig: 0.0
            attention_dropout=attention_dropout,  # orig: 0.1
            stochastic_depth=stochastic_depth,    # orig: 0.1
            num_classes=n_output_channels,
            num_heads=num_heads,
            mlp_ratio=mlp_ratio,
            *args, **kwargs)
    ...
    def forward(self, x):
        tokens = self.tokenizer(x)      # B x #patches x embedding_dim
        out_d = self.classifier(tokens)
        
        return {
            'out': out_d['patch_out'],
            'patch_tokens': tokens,
            'mha_in': out_d['mha_in'],
            'patch_outs': out_d['patch_outs'],
            'global_out': out_d['global_out'],
            'input_shape': x.shape
        }
    ...
    def init_weight(m):
        if isinstance(m, nn.Linear):
            ...
            nn.init.trunc_normal_(m.weight, std=.02)
            if isinstance(m, nn.Conv1d) and exists(m.bias):
                nn.init.constant_(m.bias, 0)
