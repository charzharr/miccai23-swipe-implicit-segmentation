...
class ReweighAccumulator3(BaseModel):
    ...
    def __init__(self, patch_emb_dim, hidden_dim, use_rfb=True):
        ...
        # Added 3/20/23
        self.conversion = nn.Sequential(
            nn.BatchNorm3d(patch_emb_dim),
            nn.ReLU(True),
            nn.Conv3d(patch_emb_dim, patch_emb_dim, 1, 1, bias=True),
        )
    ...
    def forward(self, features_list):
        ...
        return self.conversion(out + out1) 
        # return out + out1 
...
class ShapeMapperV33d(BaseModel):
    ...
    def __init__(
        ...
        self.set_setting('accumulator_name', accumulator)
        self.set_setting('use_rfb_lite', use_rfb_lite)
        self.set_setting('separate_classes', separate_classes)
        ...
        def get_accumulator(self, patch_emb_channels, hidden_channels):
            if 'conv' in self.accumulator_name.lower():
                accumulator = ConvAccumulator(patch_emb_channels, 
                                              hidden_channels,
                                              use_rfb=self.use_rfb)
            elif 'weigh' in self.accumulator_name.lower():
                if self.accumulator_name.lower() == 'weigh2':
                    accumulator = ReweighAccumulator2(patch_emb_channels,
                                                      hidden_channels,
                                                      use_rfb=self.use_rfb)
                elif self.accumulator_name.lower() == 'weigh3':
                    accumulator = ReweighAccumulator3(patch_emb_channels,
                                                      hidden_channels,
                                                      use_rfb=self.use_rfb)
                else:
                    accumulator = ReweighAccumulator(patch_emb_channels,
                                                     hidden_channels,
                                                     use_rfb=self.use_rfb)
            elif 'se' in self.accumulator_name.lower():
                accumulator = SEAccumulator(patch_emb_channels,
                                            hidden_channels,
                                            use_rfb=self.use_rfb)
            elif 'sa' in self.accumulator_name.lower():
                accumulator = SAAccumulator(patch_emb_channels,
                                            hidden_channels,
                                            use_rfb=self.use_rfb)
            return accumulator
        ...
        if self.separate_classes:
            self.accumulator = []
            for ci in range(14):
                self.accumulator.append(get_accumulator(self, 
                                                        patch_emb_channels,
                                                        hidden_channels))
            self.accumulator = nn.ModuleList(self.accumulator)
        else:
            self.accumulator = get_accumulator(self, patch_emb_channels,
                                               hidden_channels)
    ...
    def forward(self, features_list):
        ...
        if self.separate_classes:
            targ_emb_h = x4.shape[-1] * 4 // self.patch_size
            if feats[-1].shape[-1] != targ_emb_h:
                for i in range(len(feats)):
                    feats[i] = F.interpolate(feats[i], 
                                             size=[targ_emb_h]*3,
                                             mode='trilinear',
                                             align_corners=False)
            patch_embeddings_l = []
            for accum in self.accumulator:
                patch_embeddings_l.append(accum(feats))
            # patch_embeddings_l = torch.split(patch_embeddings, 
            #                                  self.patch_emb_channels, dim=1)
            flat_pe_l = []
            for patch_emb in patch_embeddings_l:
                flat_pe_l.append(rearrange(patch_emb, 
                                           'B C D H W -> B (D H W) C'))
            return patch_embeddings_l, flat_pe_l
